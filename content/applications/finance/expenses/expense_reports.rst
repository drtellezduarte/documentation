...
that still need to be added to an expense report) appear in blue text. All other statuses
...
First, select the desired expense to be added to the report on the :guilabel:`My Expenses`
dashboard, by ticking the checkbox next to each entry.
...
Summary`, by default. It is recommended to edit this field with a short summary for each report to
...
.. note::
   The :guilabel:`Company` field only appears if in a multi-company database.

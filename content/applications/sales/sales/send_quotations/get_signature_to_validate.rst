...
----------------------------------------
   ...
   Clients can view the online signature in *Developer Mode*. To access *Developer Mode*, open the
   **Settings** app scroll down to the **Developer Tools** section, and click **Activate the
   developer mode**.

   To view a online signature from a paid invoice, go to :menuselection:`Sales app--> Orders-->
   Orders` and select the desired sales order. A new tab,:guilabel:`Customer Signature`, is
   available. Click the tab to view the electronic signature as well as the :guilabel:`Signed By`
